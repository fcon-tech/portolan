...
```bash
go run ./cmd/portolan context prepare --root testdata/landscape-map --out /tmp/portolan-hypothesis-context --profile cursor --force
```

Cursor-assisted lane:

```bash
agent -p --mode=ask --trust --workspace /home/governor/projects/sdp/portolan '<prompt with /tmp/portolan-hypothesis-context>'
```
