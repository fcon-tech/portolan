Evaluation is based only on direct inspection of `/home/governor/projects/bigtop-landscape` (excluding `run/*` Portolan outputs). The prompt file path cited in your query is **not present** under this target (`cannot_verify` locally).
...
- Sets `require_full_corpus: true` and points `corpus_manifest` to a path **outside** this target (`/home/governor/projects/sdp/portolan/corpora/apache-bigtop/manifest.json`). Full-corpus membership and missing upstream repos are **`unknown`** from target-only inspection.
...
- Spec prompt file at `specs/034-cursor-comparison-validation/...` (**not present** in target).
