...
**Local scope (observed):** Portolan mapped `/home/governor/projects/bigtop-landscape` and found **18 Git repositories** under `repos/`, all `source-visible` with local paths present (`summary.json` coverage, `repos.json`, `evidence-index.jsonl`).
