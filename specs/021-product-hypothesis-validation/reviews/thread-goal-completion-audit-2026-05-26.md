...
| --- | --- | --- |
| OSS assembly specs formulated and implemented | `specs/018-oss-agent-context-assembly/`, `specs/022-oss-tool-output-assembly/`, `specs/023-relationship-surface-assembly/`, `specs/025-oss-execution-plan/`, `specs/031-oss-adapter-contract/`, `specs/032-context-evidence-index/`; docs `docs/research/2026-05-26-large-codebase-oss-landscape.md`, `docs/oss-composition.md`; CLI `context prepare`, `adapter validate`; fixtures under `testdata/oss-adapter-contract/`. | verified for local context/adapter assembly |
| Portolan improvement and pruning specs formulated and implemented | `specs/019-portolan-scope-pruning/`, `specs/024-agent-scale-map-summary/`, `specs/028-large-findings-jsonl/`, `specs/029-bounded-graph-index/`, `specs/030-graph-slice-command/`, `specs/033-agent-command-guardrails/`; primary workflow moved from prepared `selection.json` to `context prepare` and `map --root`; raw graph drill-down bounded by `summary.json`, `graph-index.json`, and `graph slice`. | verified for current repo-local workflow |
| Cursor skill specs formulated and implemented | `specs/020-cursor-agent-skill-set/`; `.cursor/rules/portolan-map.mdc`; `agent/START_HERE.md`; `agent/AGENT_GUIDE.md`; `agent/skills/portolan-map/SKILL.md`; generated `answer-contract.md` and `query-plan.md`; command guardrails in `specs/033-agent-command-guardrails/`. | verified for headless Cursor Agent and portable skill surface |
| Product hypotheses formulated and checked | `specs/021-product-hypothesis-validation/spec.md` H1-H10; ledgers under `specs/021-product-hypothesis-validation/reviews/`; fresh H10 run against `/home/governor/projects/vibe_coding` with `/tmp/portolan-h10-context-guarded` and `/tmp/portolan-h10-map`. | verified for current H1-H10 cycle |

## Fresh Acceptance Evidence

- `verified`: `portolan context prepare --root /home/governor/projects/vibe_coding --out /tmp/portolan-h10-context-guarded --profile cursor --force`.
...
- `verified`: `portolan map --root /home/governor/projects/vibe_coding --out /tmp/portolan-h10-map --force`.
