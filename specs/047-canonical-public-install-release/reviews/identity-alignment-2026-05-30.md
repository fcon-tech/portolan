...
- Internal Go imports changed from `github.com/governor/portolan/...` to
...
| stale Go import scan | verified | `rg -n "github.com/governor/portolan" --glob '*.go' .` returned no matches. |
| public identity scan | verified | `rg -n "github.com/(fcon-tech|governor)/portolan|go install|git clone|Homebrew|brew install|Docker|docker pull|npm install|apt install|yum install" README.md docs go.mod internal -S` showed `fcon-tech` install/clone/ldflags paths and no old public install path. Docker matches were in Bigtop/configuration technical docs, not package-manager install claims. |
