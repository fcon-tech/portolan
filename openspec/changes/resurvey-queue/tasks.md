...
- [ ] 1.1 A small pure helper (new leaf module under `core/src/`):
...
- [ ] 1.2 Tests: cross-vessel fairways count; intra-vessel fairways do
...
- [ ] 2.1 `core/src/harbor/proposals.ts`: replace the grouped
...
- [ ] 2.2 Queue sort: repair rows order by the shared rank (fan-in
      desc, vessel id asc) before the kind rank resolves against
      new-land and gap (kind rank unchanged: repair > new-land > gap).
- [ ] 2.3 Tests (from the spec scenarios): two drifted vessels produce
...
- [ ] 2.4 Existing harbor tests updated where they assumed the grouped
      row; `decide`/`run` flows stay green on per-vessel fingerprints.
- [ ] 2.5 Verification: `bun test` green; `bunx tsc --noEmit` in `core/`.

## 3. Cumulative night bound

- [ ] 3.1 `core/src/harbor/night-policy.ts`: spend the bound down the
...
- [ ] 3.2 Tests: five drifted vessels, bound three — the three
...
- [ ] 5.1 `docs/MANIFEST.md` harbor row and `README.md` harbor sentence
      checked against the ranked queue (both describe the harbor
      generically today; touch only if wording now lies).
- [ ] 5.2 No machine-home paths in tracked files; no new synonyms
      (kinds stay `repair | gap | new-land`; "pending correction" stays
      the locked term).
- [ ] 5.3 Verification: `bun test`; `bunx tsc --noEmit` in `core/`;
