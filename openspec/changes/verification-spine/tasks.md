...
- [x] 1.1 Inspect Serena (README, docs, tool list, MCP surface) and
...
- [x] 1.2 Write `docs/verification-trials.md`: one row per competitor per
...
- [x] 1.3 Verification: every claim in the page carries a label and an
      anchor; no machine-home paths (leak-gate stays clean).

## 2. `trust.report` core module

- [x] 2.1 `core/src/tools/trust-report.ts`: aggregate over `readChart` —
...
- [x] 2.2 Tests (from the spec scenarios): one-call summary shape; fresh
...
- [x] 2.3 Verification: `bun test` green; `bunx tsc --noEmit` in `core/`.

## 3. Serve the tool

- [x] 3.1 Register `trust.report` in `core/src/server/registry.ts`
      (thirteenth tool, input schema `{}`); MCP wiring test: call through
      the server client.
- [x] 3.2 Verification: `bun test` green; the registry test lists thirteen
      tools.

## 4. The committed receipt

- [x] 4.1 Thin deterministic script (`scripts/`, ~20 lines) that runs the
...
- [x] 4.2 Verification: re-running the command is byte-identical;
      leak-gate clean.

## 5. Skill mandate

- [x] 5.1 `skill/SKILL.md`: Sailing Directions section instructs calling
      `trust.report` for the verification summary and reporting refuted
      anchors verbatim; tool desk gains the row.
- [x] 5.2 Verification: `bun run skill/verify/checks.ts` passes.

## 6. Positioning with receipts

- [x] 6.1 README: verification-spine paragraph; differentiation claims
      worded as "no surveyed tool" and anchored to `docs/demo/trust-report.md`
      and `docs/verification-trials.md`.
- [x] 6.2 `docs/landing.html`: same wording, same anchors.
- [x] 6.3 `docs/MANIFEST.md`: tool table gains `trust.report`; glossary row
      for the verification summary.
- [x] 6.4 Verification: every new claim in these files carries an anchor;
