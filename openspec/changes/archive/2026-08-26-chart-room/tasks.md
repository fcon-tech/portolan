...
- [x] 1.1 Port the spike into `core/src/chartroom/render.ts` +
      ...
      failure
- [x] 1.2 Add `core/src/chartroom/cli.ts render --target <t>` (stdout =
...
- [x] 2.1 Register the `chart.render` MCP tool (no arguments beyond the
...
- [x] 3.1 Extend the MANIFEST glossary with **Chart room** and verify the
      term appears exactly once, in the glossary, without touching the
      non-goals list
- [x] 3.2 Regenerate the Bigtop artifact from core and verify the
...
- [x] 3.3 Delete `experiments/map-spike/` (the visual contract now lives
