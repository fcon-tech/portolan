...
- [x] 1.1 Implement the shared sounding result (verdict enum
...
- [x] 1.2 Add the determinism harness (run a sounding twice on an unchanged
      fixture, deep-compare results) and verify it passes for both
      operations

## 2. sound.anchor

- [x] 2.1 Implement file-anchor verification (exists, line range within
...
- [x] 2.2 Implement manifest-key and receipt-id anchor verification and
      verify tests confirm a live key/receipt and refute a dead one by id

## 3. sound.edge

- [x] 3.1 Implement the manifest-declaration means (source vessel's
...
- [x] 4.2 Add an API-surface test asserting the sounding entry points
      ...
      (note: the store carries no write counters; verified equivalently —
      the sounding module imports no store writer and task 4.1's battery
      proves zero writes on disk)
