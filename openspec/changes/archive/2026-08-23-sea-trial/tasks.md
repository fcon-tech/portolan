...
- [x] 1.1 Create the gate-runner entry point under `acceptance/` taking a
...
- [x] 1.2 Implement the narrow BOM reader (name/version/dependency entries
...
- [x] 3.1 Implement the fabrication gate (sound.anchor over every cited
      ...
      answers
- [x] 3.2 Implement the unsurveyed-honesty gate (runtime topology and
...
- [x] 4.1 Implement fairway completeness (charted fairways vs BOM-derived
      dependency list, missing named) and verify a fixture test reports
      the correct ratio and missing list
- [x] 4.2 Implement trust distribution (entry count per trust label) and
      verify a fixture chart yields the exact five-way share
- [x] 4.3 Implement the staleness-flip check (single whitespace-only
...
- [x] 5.1 Implement the gate-tree verdict (FAIL on any refuted anchor,
...
- [x] 5.2 Implement the Governor's-read capture (present the designated
...
- [x] 5.3 Emit the trial report at `<target>/.portolan/sea-trial/report.md`
