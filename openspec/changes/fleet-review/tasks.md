...
- [ ] 1.1 Implement `buildFleetReview(targets)` + template in
...
- [ ] 1.2 Add the `review` CLI subcommand (repeatable `--target`, artifact
      into the first target's `.portolan/`) and verify the printed path +
      usage error when fewer than one charted target is given

## 2. Contract + archive

- [ ] 2.1 Extend the MANIFEST glossary with **Fleet review** and verify
