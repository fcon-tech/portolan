...
## Purpose
Defines Portolan's published presence: one npm package carrying core,
skill, and adapters, a single bin surface, a committed MCP Registry
manifest kept in version-lockstep with the package, a registry-based
install path with no repository clone, and a publish gate that publishes
