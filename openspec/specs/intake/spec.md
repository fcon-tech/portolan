...
the captain what they have (not a filled-in manifest), persists a typed intake
...
Intake SHALL be a managed conversation where the agent asks the captain what
...
- THEN it records an unknown rather than prompting the captain mid-build
...
The expedition perimeter SHALL be exactly what the captain named. Links leading
...
- GIVEN the captain provides only a URL with no anchors
- WHEN intake runs
- THEN the agent asks for named anchors rather than autodiscovering

### Requirement: Architectural principles are captain-sourced
Architectural principles for principle-violation checking SHALL be named by the
captain at intake. If none are given, the system MUST NOT invent principles to
