...
3. Run context preparation:
   ...
   `evidence-index.jsonl`, `repos.json`, `tool-registry.json`,
   `oss-plan.json`, and `gaps.jsonl`. Treat `evidence-index.jsonl` and
   tool-registry summaries for jscpd, CycloneDX/Syft, Backstage, OpenAPI,
   AsyncAPI, and Structurizr as local evidence candidates, not final
   architecture verdicts. Use `answer-contract.md` as the required answer
   shape. Use `oss-plan.json` to see safe local producer commands when evidence
   is missing; do not run them without approval.
