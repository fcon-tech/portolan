/**
 * Domain: landscape unit classification — promotion rule, lifecycle, type.
 *
 * Single responsibility: decide how a raw corpus target maps to a landscape
 * unit's classification (charter 08 "Component Promotion Rule", legacy-required
 * until 0.2.0). Pure functions, zero dependencies. Domain layer.
 *
 * A raw entity is promoted to a unit only when its kind has strong identity;
 * surface-only kinds are never promoted; ambiguous kinds fall back to the
 * tie-breaker upstream (keep as surface/unknown unless two independent signals
 * support it).
 */
'use strict';
