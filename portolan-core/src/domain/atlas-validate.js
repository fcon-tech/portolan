/**
 * Domain: semantic atlas validator.
 *
 * Single responsibility: enforce the invariants that JSON Schema cannot express
 * — id uniqueness across kinds, relationship endpoint resolution, surface owner
 * resolution, route/object-kind consistency, promotion-signal sufficiency,
 * referential integrity, surface eligibility, family enum + secondary dedup.
 *
 * validateSystemMap(map) -> { errors }. Pure function, zero dependencies,
 * unit-testable with in-memory maps. Domain layer. (JSON Schema via ajv is a
