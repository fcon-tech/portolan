...
Date: 2026-05-20
PR: https://github.com/governor/portolan/pull/6
...
- verified: `go run ./cmd/portolan scan --selection internal/testfixtures/black-box-profile/selection.json --out /tmp/portolan-black-box-graph.json --force`
...
- accepted/fixed: `openrouter/deepseek/deepseek-v4-pro` found that
  `docs/specs/006-evidence-diff/` was included in the PR diff even though PR #6 is
...
- accepted/documented: duplicate black-box fixtures under root `internal/testfixtures/` and
  `internal/app/testfixtures/` are retained because package tests and repository-root
  CLI commands run from different working directories.

## PR State

- verified: PR #6 exists at `https://github.com/governor/portolan/pull/6`.
