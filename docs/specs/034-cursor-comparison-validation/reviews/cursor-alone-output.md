...
- Sets `require_full_corpus: true` and points `corpus_manifest` to a path **outside** this target (`/home/governor/projects/sdp/portolan/internal/testfixtures/corpus-manifests/apache-bigtop/manifest.json`). Full-corpus membership and missing upstream repos are **`unknown`** from target-only inspection.
