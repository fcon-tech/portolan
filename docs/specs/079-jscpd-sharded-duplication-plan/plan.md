...
```bash
go run ./cmd/portolan context prepare --root /home/governor/projects/bigtop-landscape --out /home/governor/projects/bigtop-landscape/.portolan/stress/<run-id>/context --profile agent --force
jq '.tools[] | select(.id == "jscpd") | {status,evidence_state,commands: [.commands[] | {label,reads,writes}]}' /home/governor/projects/bigtop-landscape/.portolan/stress/<run-id>/context/oss-plan.json
```
