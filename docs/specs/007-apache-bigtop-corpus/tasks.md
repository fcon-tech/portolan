...
**Input**: `docs/specs/007-apache-bigtop-corpus/spec.md`
**Prerequisites**: `docs/specs/007-apache-bigtop-corpus/plan.md`

## Phase 1: Corpus Contract

- [x] T001 Add `schema/corpus-manifest.schema.json` for curated test corpus
  manifests.
- [x] T002 Add `internal/testfixtures/corpus-manifests/apache-bigtop/manifest.json` pinned to Bigtop 3.5.0
  official metadata.
...
- [x] T014 Add an initial Bigtop skill-smoke runbook under `docs/test-corpora/apache-bigtop.md` or `docs/specs/007-apache-bigtop-corpus/`.
...
- [x] T017 Record a gap ledger under `docs/specs/007-apache-bigtop-corpus/reviews/` with agent workflow failures, missing relationships, missing duplication, missing configuration surfaces, missing technical-debt findings, packet usefulness gaps, and unsupported agent inferences.
...
- [x] T019 Wait for `docs/specs/014-agent-bootstrap-discovery/` to make Portolan
  self-discoverable from generic agent inputs.
- [x] T020 Wait for `docs/specs/015-blind-agent-acceptance/` to define the allowed
...
- [x] T021 Run the blind protocol against a real local Apache Bigtop checkout.
  - Status 2026-05-26: blocked on `docs/specs/017-landscape-root-discovery/`.
  ...
  - CLI stress 2026-05-27: generic root workflow ran without `selection.json`
    against `/home/governor/projects/bigtop-landscape` and wrote context
  ...
  - Operator lane 2026-05-27: Cursor Agent CLI / Composer 2.5 ran the blind
    protocol against `/home/governor/projects/bigtop-landscape`, wrote
...
- [x] T022 If the local Apache Bigtop checkout is absent, record the run as
  ...
  - Status 2026-05-27: not applicable for the completed lane because the local
    ...
    `/home/governor/projects/bigtop-landscape`.
