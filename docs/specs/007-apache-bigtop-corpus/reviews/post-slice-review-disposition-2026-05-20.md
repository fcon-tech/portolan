...
| Local fallback `map --root .` targeted the Portolan repo rather than the fixture root. | glm | Accepted and fixed. The runbook now uses `internal/testfixtures/apache-bigtop-smoke/repo`. |
...
| Gap ledger artifact was missing from review input. | kimi | Rejected with local evidence. The artifact exists at `docs/specs/007-apache-bigtop-corpus/reviews/acceptance-smoke-ledger-2026-05-20.md`; it was not visible to that lane because the prompt used `git diff` and omitted untracked files. |
