...
| Fixture command | verified | `go run ./cmd/portolan map --root internal/testfixtures/map-command/repo --out /tmp/portolan-map-run --force` wrote the bundle. |
...
| JSON/schema syntax | verified | `jq empty schema/*.json internal/testfixtures/corpus-manifests/apache-bigtop/manifest.json` passed. |
...
| Duplicate fixture trees existed under package-local and repo-level `testdata`. | deepseek, gemini | Accepted and fixed. App tests now reference the repo-level fixture with `../../internal/testfixtures/map-command/repo`; the duplicate package-local fixture was removed. |
