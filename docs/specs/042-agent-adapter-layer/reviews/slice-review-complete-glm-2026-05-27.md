...
- severity: minor
  evidence: "The diff adds the fixture `graphify-minimal.json` but does not show a corresponding `internal/testfixtures/oss-adapter-contract/graphify-minimal.json` schema validation test (e.g. `jq empty` or JSON Schema check) beyond the Go-level `ValidateFile` call."
