...
**Portolan command:** `/home/governor/projects/sdp/portolan-051-portolan-quality-boundary/.portolan/bin/portolan` (`portolan dev`) via `scripts/bootstrap-portolan` — no system-wide `portolan` binary  
...
**Answer:** One locally visible Git repository (`root` → `/home/governor/projects/vibe_coding/spec-kit`) with bounded file/config inventory; ecosystem completeness and most cross-service semantics are explicitly incomplete.
