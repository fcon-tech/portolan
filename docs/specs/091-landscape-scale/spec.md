...
**Input**: Full bigtop landscape (18 repos) through orient-wizard without OOM abort;
shard failures as honest gaps; scalable bundle build with kind-aware budget.

## Requirements

- **FR-001**: `--jscpd-memory-mb` and `--shard-timeout` on orient-wizard.sh.
- **FR-002**: Per-shard failures written to `producers/_gaps.jsonl`.
- **FR-003**: Single-pass jq per producer file in build-orient-bundle.sh.
