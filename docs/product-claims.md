...
| Portolan composes existing OSS tool outputs instead of reimplementing mature scanners. | `narrowed` | Safe only for named local producer/import paths and recorded evidence. Graphify MCP/LLM/dashboard behavior, SCIP protobuf/real indexer output, real Serena export/MCP behavior, Repomix source/redaction semantics, broad Semgrep rule value, and broad OSS producer value remain unassessed. |
| Portolan detects duplication across a landscape. | `narrowed` | Safe for native exact source/config duplicate clusters and bounded jscpd near-clone evidence only for the named target/profile that produced usable JSON. Broad near-clone and component-duplication claims remain unproven. |
