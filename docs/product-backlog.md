...
| P6-061 | `docs/specs/061-bigtop-runtime-capture-approval/` | A safe, reversible Bigtop runtime capture path is defined against upstream Bigtop provisioner tooling, including approval boundary, cleanup plan, resource/network risk, and acceptable runtime-visible producer outputs before any cluster is started. | Ready-for-review PR candidate; upstream Bigtop Docker provisioner selected as the minimal runtime capture candidate, approval/runbook boundary recorded, Cursor Composer 2.5 stress verified `cannot_verify`/`blocked` boundaries, three assessed non-GPT review lanes completed, local baseline passed; no runtime provisioning approved or executed, Bigtop runtime topology remains `cannot_verify`, enterprise parity remains not_assessed |
