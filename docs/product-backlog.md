...
| ID | Spec | Outcome | Status |
| --- | --- | --- | --- |
| P10-109 | `docs/specs/109-evidence-promotion-stratified-atlas/` | Raw evidence, classified sources, promoted facts, findings, claims, health, and lazy raw artifacts are separated across every Portolan evidence family so polluted or incomplete bundles cannot look like complete architecture truth. | PR #73 ready-for-review; local implementation verified, three independent PR review lanes assessed with accepted findings fixed, current-head GitHub Baseline refresh pending after status cleanup, merge not ready |
