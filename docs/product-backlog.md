...
| P6-072 | `docs/specs/072-existing-artifact-jdeps/` | Existing compiled JVM artifacts already present in selected Bigtop targets are analyzed with `jdeps` to produce bounded class/package dependency evidence without building targets or installing new indexers. | Ready-for-review PR #50 after closeout refresh; `jdeps` 26.0.1 ran read-only over 9 existing `.jar`/`.class` artifacts from Apache Zeppelin, Hive, and Bigtop test/resource paths, producing 289 package dependency rows with 16 unresolved `not found` rows; artifact evidence is dominated by bundled third-party test/resource jars and tiny UDF fixtures, not Bigtop production build outputs; Cursor stress, three assessed non-GPT review lanes, local baseline, and initial GitHub checks passed; evidence is bounded compiled-artifact dependency output only, while full symbol/reference graph, call graph, runtime topology, and enterprise parity remain `cannot_verify` |
