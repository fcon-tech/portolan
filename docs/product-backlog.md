...
| P6-062 | `docs/specs/062-bigtop-runtime-capture-preflight/` | The approved Bigtop runtime capture path is preflighted without starting containers, and the exact remaining approval gate for a single-node runtime capture is recorded with command/output evidence. | Ready-for-review PR candidate; Docker/Compose/Ruby/cgroup and Bigtop `--env-check` preflight passed read-only, Cursor Composer 2.5 stress preserved prerequisites as `verified`, runtime topology as `cannot_verify`, and next create command as `blocked`, three assessed non-GPT review lanes completed, local baseline passed; runtime provisioning still not approved or executed |
