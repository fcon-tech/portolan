...
| P6-063 | `docs/specs/063-bigtop-semgrep-local-producer/` | Semgrep is rerun with a local no-registry rule pack against bounded Bigtop deployment/provisioner surfaces so API/catalog mention evidence beyond Syft/CycloneDX is verified without telemetry, network rule fetches, or runtime claims. | Ready-for-review PR candidate; bounded Semgrep local-config producer run succeeded with 143 findings across 102 scanned files and 0 Semgrep errors, rule pack/hash/size evidence is auditable, Cursor Composer 2.5 stress preserved metadata-only boundaries, three assessed non-GPT review lanes completed, local baseline passed; runtime topology, full symbol/reference graph, call graph, full corpus coverage, and enterprise parity remain `cannot_verify` |
