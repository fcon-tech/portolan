...
| P6-067 | `docs/specs/067-bigtop-compose-model-producer/` | Bigtop Docker provisioner Compose files are normalized through `docker compose config` into real deployment-model producer outputs without starting containers. | Ready-for-review PR candidate; Docker Compose v5.1.4 generated cgroup v1/v2 model outputs for the Bigtop provisioner, resolving service, network, mount, privilege, image, and memory-limit metadata, Cursor Composer 2.5 preserved outputs as metadata-visible deployment-model evidence only, three assessed non-GPT review lanes completed, local baseline passed; runtime topology remains `cannot_verify` until an approved runtime capture observes running containers |
