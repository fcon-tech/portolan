...
| P6-055 | `docs/specs/055-runtime-topology-evidence/` | Runtime topology evidence is imported only from explicit local runtime-visible observations, with dependency/catalog/static signals kept separate from runtime claims. | Local implementation in progress; top-level runtime observation import verified on fixture map smoke and Cursor Composer 2.5 stress; static dependency evidence remains metadata-visible; Bigtop runtime topology remains blocked/not_assessed because no safe local runtime observation export is selected |
