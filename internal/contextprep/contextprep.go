...
func renderAnswerContract(root string) string {
	...
	fmt.Fprintf(&b, "| Declared service/API | `metadata-visible` | Local catalogs, OpenAPI/AsyncAPI, Structurizr, manifests, or imported tool output declare intended relationships. | That the declaration is current runtime behavior. |\n")
	...
	fmt.Fprintf(&b, "Relationship answers must name both relationship kind and evidence type. `source-visible` and `metadata-visible` records do not prove runtime traffic; runtime topology is `not_assessed` unless runtime-visible local observations were supplied and inspected.\n\n")
