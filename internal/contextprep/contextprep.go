...
func renderAnswerContract(root, out string) string {
	...
	fmt.Fprintf(&b, "| --- | --- | --- | --- |\n")
	fmt.Fprintf(&b, "| Source dependency | `source-visible` or `metadata-visible` | Local source files or manifests show code/package coupling. | Runtime communication, service topology, or production behavior. |\n")
	fmt.Fprintf(&b, "| Declared service/API | `metadata-visible` | Local catalogs, OpenAPI/AsyncAPI, Structurizr, manifests, or imported tool output declare intended relationships. | That the declaration is current production behavior. |\n")
	...
	fmt.Fprintf(&b, "External tool evaluation profiles are adoption guidance, not evidence. Read `docs/adapter-contracts/external-tool-evaluation-profiles.md` before recommending CodeGraph, Understand-Anything, or ast-index, and treat the profile as stale if its `Last refreshed` date is older than the current review window. Current guidance: ast-index is the strongest current symbol/reference producer candidate, CodeGraph is lower-fit optional, and Understand-Anything is UX inspiration. Do not promote candidate profiles to source-visible, metadata-visible, or runtime-visible evidence; local output and a Portolan import/evaluation contract are required first.\n\n")
...
- Dependency producer evidence: if dependency/build manifests are visible but
  CycloneDX/build-tool outputs are missing, inspect ` + "`oss-plan.json`" + ` for
  build-system, SBOM, or CycloneDX-compatible plans. Treat them as
  approval-gated native producer options, not Portolan-owned stack adapters,
  and keep dependency relationships ` + "`not_assessed`" + ` until local output exists.
